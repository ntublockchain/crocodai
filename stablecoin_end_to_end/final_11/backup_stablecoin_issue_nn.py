...
# bytes_0 = deployed_relay_contract.functions.getBytes().call()
# print("bytes from contract: "+bytes_0.hex())
# hash_1 = deployed_relay_contract.functions.getHashBytes().call()
# print("hash from contract: "+hash_1.hex())

k = sha3.keccak_256()
k.update(bytearray.fromhex("aaa".encode('utf-8').hex()))
print("test hash: "+k.hexdigest())

addvote_relay_nonce = w3.eth.get_transaction_count(w3.eth.accounts[0])

signing_key = ecdsa.SigningKey.from_string(bytes.fromhex(private_keys[0]), ecdsa.SECP256k1, sha3.keccak_256)
signature = signing_key.sign_deterministic(bytearray.fromhex("aaa".encode('utf-8').hex()))
print(signing_key.get_verifying_key().to_string())
# signature = signing_key.sign_digest(bytes.fromhex(message_hash))
# signature2 = signing_key.sign_deterministic(msg_raw.encode('utf-8'))
# signature3 = signing_key.sign_deterministic(hashlib.sha3_256(msg_raw.encode('utf-8')).digest())
print("signature:", signature.hex())
# print("signature2:", signature2.hex())
# print("signature3:", signature3.hex())
sig_variant_00 = signature.hex()+"00"
# vk = signing_key.get_verifying_key()
# print(vk.verify(signature2, msg_raw.encode('utf-8')))

# w3.eth.accounts.hashMessage("aaa")
# msg_w3 = encode_defunct(Web3.solidity_keccak(['string'], ["aaa"]))
# print(msg_w3)
signed_message =  w3.eth.account.sign_message(encode_defunct(text="aaa"), private_key=private_keys[0])
# w3_signature = signed_message["signature"]
print(signed_message)
signature = signed_message["signature"]

# signature4 = signing_key.sign_deterministic(hashlib.sha256("\x19Ethereum Signed Message:\n32".encode('utf-8')).digest())
# print("signature4:", signature4.hex())

addvote_relay_nonce = w3.eth.get_transaction_count(w3.eth.accounts[0])
deployed_relay_contract.functions.approve(cctransfer_address,latest_index,signature).transact({"chainId": chain_id, "gasPrice": w3.eth.gas_price, "from": w3.eth.accounts[0], "nonce": addvote_relay_nonce})
