...
message = w3.keccak(hexstr = (cctransfer_address + format(latest_index,"064x")))

server_process = subprocess.Popen([sig_dir+'gg20_sm_manager'], shell = True, stdout=subprocess.PIPE)

sign_processes = [None] * n
signer_string = '1'
for i in range(n-t-1):
    signer_string = signer_string + ',' + str(i+2)

for i in range(n-t):
    sign_processes[i] = subprocess.Popen([sig_dir+'gg20_signing', '-p', signer_string, '-d', message.hex()[2:], '-l', 'local-share'+str(i+1)+'.json'], shell = True, stdout=subprocess.PIPE)
    time.sleep(0.1)

for i in range(n-t):
    sign_processes[i].wait()
    
server_process.kill()

signature_string = ""
for line in sign_processes[0].stdout:
    line_text = line.decode("utf-8").strip()
    x = re.split(": ", line_text)
    if len(x) > 1:
        signature_string = x[1]
        
print("signature:", signature_string)
...
deployed_relay_contract.functions.approve(cctransfer_address,latest_index,bytearray.fromhex(signature_string)).transact({"chainId": chain_id, "gasPrice": w3.eth.gas_price, "from": w3.eth.accounts[0], "nonce": addvote_relay_nonce})
