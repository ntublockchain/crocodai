...
with open(os.path.join(__location__, "Relay.sol"), "r") as file:
    relay_2_contract_file = file.read()

compiled_relay_2_sol = compile_standard(
    {
        "language": "Solidity",
        "sources": {"Relay.sol": {"content": relay_2_contract_file}},
        "settings": {
            "outputSelection": {
                "*": {
                    "*": [
...
with open(os.path.join(__location__, "compiler_output.json"), "w") as file:
    json.dump(compiled_relay_2_sol, file)

## get bytecode
relay_2_bytecode = compiled_relay_2_sol["contracts"]["Relay.sol"]["Relay"]["evm"][
    "bytecode"
]["object"]

# ## get abi
relay_2_abi = json.loads(
    compiled_relay_2_sol["contracts"]["Relay.sol"]["Relay"]["metadata"]
)["output"]["abi"]

# create the contract in Python
relay_2_contract = w3.eth.contract(abi=relay_2_abi, bytecode=relay_2_bytecode)
# get the latest transaction
relay_2_nonce = w3.eth.get_transaction_count(node_address)

# create a transaction that deploys the contract
deploy_relay_2_transaction = relay_2_contract.constructor().build_transaction(
    {"chainId": chain_id, "gasPrice": w3.eth.gas_price, "from": node_address, "nonce": relay_2_nonce}
)

# sign the transaction
signed_deploy_relay_2_transaction = w3.eth.account.sign_transaction(deploy_relay_2_transaction, private_key=node_private_key)
print(f"Start Deploying Relay Contract!")
# send the transaction
deploy_relay_2_transaction_hash = w3.eth.send_raw_transaction(signed_deploy_relay_2_transaction.rawTransaction)
# wait for the transaction to be mined, and get the transaction receipt
print("Waiting for transaction to finish...")
deploy_relay_2_transaction_receipt = w3.eth.wait_for_transaction_receipt(deploy_relay_2_transaction_hash)
relay_2_address = deploy_relay_2_transaction_receipt.contractAddress
print(f"Done! Contract Relay deployed to {relay_2_address}")

deployed_relay_2_contract = w3.eth.contract(relay_2_address, abi=relay_2_abi)
...
with open(os.path.join(__location__, "CCTransfer.sol"), "r") as file:
    cctransfer_2_contract_file = file.read()

compiled_cctransfer_2_sol = compile_standard(
    {
        "language": "Solidity",
        "sources": {"CCTransfer.sol": {"content": cctransfer_2_contract_file}},
        "settings": {
            "outputSelection": {
                "*": {
                    "*": [
...
with open(os.path.join(__location__, "compiler_output.json"), "w") as file:
    json.dump(compiled_cctransfer_2_sol, file)

## get bytecode
cctransfer_2_bytecode = compiled_cctransfer_2_sol["contracts"]["CCTransfer.sol"]["CCTransfer"]["evm"][
    "bytecode"
]["object"]

# ## get abi
cctransfer_2_abi = json.loads(
    compiled_cctransfer_2_sol["contracts"]["CCTransfer.sol"]["CCTransfer"]["metadata"]
)["output"]["abi"]

# create the contract in Python
cctransfer_2_contract = w3.eth.contract(abi=cctransfer_2_abi, bytecode=cctransfer_2_bytecode)
# get the latest transaction
cctransfer_2_nonce = w3.eth.get_transaction_count(node_address)

# create a transaction that deploys the contract
deploy_cctransfer_2_transaction = cctransfer_2_contract.constructor(coin_address, relay_address).build_transaction(
    {"chainId": chain_id, "gasPrice": w3.eth.gas_price, "from": node_address, "nonce": cctransfer_2_nonce}
)

# sign the transaction
signed_deploy_cctransfer_2_transaction = w3.eth.account.sign_transaction(deploy_cctransfer_2_transaction, private_key=node_private_key)
print(f"Start Deploying CCTransfer Contract!")
# send the transaction
deploy_cctransfer_2_transaction_hash = w3.eth.send_raw_transaction(signed_deploy_cctransfer_2_transaction.rawTransaction)
# wait for the transaction to be mined, and get the transaction receipt
print("Waiting for transaction to finish...")
deploy_cctransfer_2_transaction_receipt = w3.eth.wait_for_transaction_receipt(deploy_cctransfer_2_transaction_hash)
cctransfer_2_address = deploy_cctransfer_2_transaction_receipt.contractAddress
print(f"Done! Contract CCTransfer deployed to {cctransfer_2_address}")

deployed_cctransfer_2_contract = w3.eth.contract(cctransfer_2_address, abi=cctransfer_2_abi)

##############################################
####### Set up the contract parameters #######
##############################################

### --- set up the CroCoDai relay --- ###
...
### CREATE AND APPROVE OUTGOING CC TRANSFER REQUEST ###

print("Allow CCTransfer to receive 10 Dai...")
nonce = w3.eth.get_transaction_count(node_address)
tx_hash = deployed_coin_contract.functions.approve(cctransfer_address,10).transact({"chainId": chain_id, "gasPrice": w3.eth.gas_price, "from": node_address, "nonce": nonce})
print(f"Done! Allowed CCTransfer to receive 10 Dai")
...
tx_hash = deployed_cctransfer_contract.functions.addOutgoingRequest(10).transact({"chainId": chain_id, "gasPrice": w3.eth.gas_price, "from": node_address, "nonce": add_request_nonce})
...
print("Approve cross-chain transfer request...")
add_request_nonce = w3.eth.get_transaction_count(node_address)
tx_hash = deployed_relay_contract.functions.approve(cctransfer_address,0).transact({"chainId": chain_id, "gasPrice": w3.eth.gas_price, "from": node_address, "nonce": add_request_nonce})
print(f"Done! Approved cross-chain request")
...
### CREATE AND APPROVE INCOMING CC TRANSFER REQUEST ###

print("Create cross-chain transfer request...")
add_request_nonce = w3.eth.get_transaction_count(node_address)
tx_hash = deployed_cctransfer_2_contract.functions.addIncomingRequest(10,node_address).transact({"chainId": chain_id, "gasPrice": w3.eth.gas_price, "from": node_address, "nonce": add_request_nonce})
print(f"Done! Added cross-chain request")
...
print("Approve cross-chain transfer request...")
add_request_nonce = w3.eth.get_transaction_count(node_address)
tx_hash = deployed_relay_2_contract.functions.approve(cctransfer_2_address,0).transact({"chainId": chain_id, "gasPrice": w3.eth.gas_price, "from": node_address, "nonce": add_request_nonce})
print(f"Done! Approved cross-chain request")
