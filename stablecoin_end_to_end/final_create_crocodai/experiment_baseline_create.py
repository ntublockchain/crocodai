...
print("Allow BaseCoin Bridge to receive 100 BaseCoin...")
nonce = w3.eth.get_transaction_count(node_address)
tx_hash = deployed_basecoin_contract.functions.approve(basecoin_bridge_address,100).transact({"chainId": chain_id, "gasPrice": w3.eth.gas_price, "from": node_address, "nonce": nonce})
print(f"Done! Allowed BaseCoin Bridge to receive 100 BaseCoin")

receipt = w3.eth.wait_for_transaction_receipt(tx_hash)
print(f"Gas use: {receipt.gasUsed}")
step1_gas = receipt.gasUsed

print("Send 100 BaseCoin to the bridge...")
nonce = w3.eth.get_transaction_count(node_address)
tx_hash = deployed_basecoin_bridge_contract.functions.deposit(100,"Chain B",node_address).transact({"chainId": chain_id, "gasPrice": w3.eth.gas_price, "from": node_address, "nonce": nonce})
print(f"Done! Sent 100 BaseCoin to the bridge")
...
print(f"Total gas use (step 5): {step5_gas}")
...
receipt = w3.eth.wait_for_transaction_receipt(tx_hash)
print(f"Gas use (step 6): {receipt.gasUsed}")
step6_gas = receipt.gasUsed
...
print(f"Total gas use (step 4): {step4_gas}")
print(f"Total gas use (step 5): {step5_gas}")
print(f"Total gas use (step 6): {step6_gas}")

print(f"Total gas use (coins: {step1_gas + step6_gas}")
print(f"Total gas use (relay): {step2_gas + step3_gas + step4_gas + step5_gas}")