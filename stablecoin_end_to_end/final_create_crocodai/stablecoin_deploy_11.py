...
### GENERATE THE THRESHOLD SIGNATURE PARAMETERS ###

sig_dir = __location__+"/../../../rust/multi-party-ecdsa/target/release/examples/"
...
for i in range(n):
    if(os.path.exists('local-share'+str(i+1)+'.json')):
        # rm_processes[i] = subprocess.Popen(['rm', 'local-share'+str(i+1)+'.json'], stdout=subprocess.PIPE)
        rm_processes[i] = subprocess.Popen(['del', 'local-share'+str(i+1)+'.json'], shell = True)
    
for i in range(n):
    if(rm_processes[i] != None):
        rm_processes[i].wait()

# generate the keys

server = subprocess.Popen([sig_dir+'gg20_sm_manager'], shell = True, stdout=subprocess.PIPE)

keygen_processes = [None] * n
for i in range(n):
    keygen_processes[i] = subprocess.Popen([sig_dir+'gg20_keygen' ,'-t', str(t), '-n', str(n), '-i', str(i+1), '--output', 'local-share'+str(i+1)+'.json'], shell = True, stdout=subprocess.PIPE)
    time.sleep(0.1)

for i in range(n):
    keygen_processes[i].wait()

server.kill()

pub_key_string = ""
for line in keygen_processes[0].stdout:
    line_text = line.decode("utf-8").strip()
    x = re.split(": ", line_text)
    if len(x) > 1:
        pub_key_string = x[1]

for i in range(n):
    keygen_processes[i].kill()

signer_address = w3.to_checksum_address(w3.keccak(hexstr=pub_key_string).hex()[-40:])
        
print("key generation completed")
print("public key:", pub_key_string)
print("address:", signer_address)
...
# create a transaction that deploys the contract
deploy_relay_transaction = relay_contract.constructor(signer_address).build_transaction(
